function create_map() {
  am4core.ready(function() {
    ...
    // the identification is based on the region of the region provided by the cloud providers
    loc = [get_location(podinfo)];

    //load prvious locations of webpage
    try {
      var prev_loc = localStorage.getItem("prev_loc");
      JSON.parse(prev_loc);
    } catch (e) {
      prev_loc = [];
    }

    //add new loaction to prevous locations
    locs = loc.concat(prev_loc);
    console.log(locs);

    imageSeries.data = [locs[0]];

    localStorage.setItem("prev_loc", JSON.stringify(locs));
